...
const PRESET_DAILY_TASKS = {
  Breakfast: ["Prepare Coffee Mug & Spoon", "Clean Salt & Pepper Shaker", "Prepare Tissue & Toothpick", "Prepare Pocket Spoon", "Clean Baby Chairs", "Check Cushion / Pillow", "Refill Sugar Bowl", "Prepare Cutlery", "Lay Out Tables & Chairs", "Clean Back Area"],
  LunchDinner: ["Menu & Drink List", "Prepare & Change Astray", "Table Booking", "Sofa Area / Station Host / Outside", "Prepare Station Service", "Prepare Cutlery", "Salt & Pepper Shaker", "Prepare Set Up Nipkin", "Glass Were (Water)", "Tissue Glass & Thoothspick", "Clean Table & Chair", "Pocket Spoon Buffet Line", "Prepare Food Runner"]
};
...
async function randomAssignDailyJobs(){
  ...
  try{
    ...
    // create tasks documents (one per template)
    const batch = db().batch();
    let i = 0;
    for(const t of tasksShuffled){
      const assignee = staffShuffled[i % staffShuffled.length];
