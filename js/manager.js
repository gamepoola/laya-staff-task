async function loadManagerPage(){
  ...
  if (el("createStaffBtn")) el("createStaffBtn").onclick = createStaff;
  if (el("createTaskBtn")) el("createTaskBtn").onclick = createTask;
  if (el("copyYesterdayAllBtn")) el("copyYesterdayAllBtn").onclick = copyYesterdayAllTasks;
  if (el("assignMode")) el("assignMode").onchange = onAssignModeChange;
...
async function createTask(){
  ...
  try{
    ...
    const forDate = (el("taskDate") && el("taskDate").value) ? el("taskDate").value : todayStr();
...
async function copyYesterdayAllTasks(){
  const targetDate = (el("taskDate") && el("taskDate").value) ? el("taskDate").value : todayStr();
  const sourceDate = addDays(targetDate, -1);

  try{
    // load yesterday tasks (daily) - must have forDate
    const snap = await db().collection("tasks").where("forDate","==",sourceDate).get();
    const sourceTasks = snap.docs.map(d=>({id:d.id, ...d.data()})).filter(t => t.active !== false);

    if(!sourceTasks.length){
      toast(`ไม่พบงานของเมื่อวาน (${sourceDate}) (ต้องเป็นงานที่มี Task date)`, "danger");
      return;
    }

    // avoid duplicates: disable existing copied tasks for targetDate
    const todaySnap = await db().collection("tasks").where("forDate","==",targetDate).get();
    const existing = todaySnap.docs.filter(d=>{
      const t = d.data() || {};
      return t.copiedFromDate || t.autoGenerated === true;
    });

    if(existing.length){
      const ok = confirm(`พบงานของวันที่ ${targetDate} ที่เป็น auto/copy อยู่แล้ว ${existing.length} งาน\nต้องการปิดของเดิมแล้วค่อยคัดลอกใหม่ไหม?`);
      if(!ok) return;
    ...
    await batch.commit();
    toast(`คัดลอกงานเมื่อวาน → ${targetDate} สำเร็จ ✅ (${count} tasks)`);
    await loadTaskList();
    await loadNotSubmittedReport();
  }catch(e){
    console.error(e);
